        ...
        <div className='flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4'>
            ...
            openNavigation ? "flex" : "hidden"
                ...
                <div className='relative z-3 flex flex-col items-center justify-center m-auto lg:flex-row'>
                    {navigation.map((item)=>(
                     <a onClick={handleCllck} key={item.id} href={item.url}
                      className={`block font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-1 ${item.onlyMobile?'lg:hidden':''
                      } p-6 md:py-8 lg:-mr-0.25 lg:text-xs lg:font-semibold ${item.url === pathname.hash?  "z-2 lg:text-n-1"
